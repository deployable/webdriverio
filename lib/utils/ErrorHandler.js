...
class ErrorHandler extends Error {
    constructor (type, msg, details) {
        ...
        if (this.seleniumStack instanceof Error) {
            let err = this.seleniumStack

            // Standard node sys error
            if (err.code && err.errno && err.syscall) {
                if (err.type === undefined) err.type = `${err.code} ${err.syscall}`
            }

            if (err.type === undefined) {
                err.type = 'GENERAL'
            } else {
                this.message = `${this.message} - ${err.type}`
            }
        }
    }
...
let RequestError = function (msg, details) {
    return new ErrorHandler('RequestError', msg, details)
}

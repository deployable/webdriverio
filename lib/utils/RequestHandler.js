...
import { RuntimeError, RequestError } from './ErrorHandler'
        ...
        return new Promise((resolve, reject) => {
            request(fullRequestOptions, (err, response, body) => {
                ...
                if (retryCount >= totalRetryCount) {
                    ...
                    if (err && err.message && err.message.indexOf('Nock') > -1) {
                        // for better unit test error output
                        error = err
                    } else if (err) {
                        error = new RequestError('Couldn\'t connect to selenium server', err)
                    } else {
